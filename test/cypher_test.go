...
func TestRenderSimpleQuery(t *testing.T) {
	device := cypher.CypherNewNode("Device").NamedByString("d").WithRawProperties("entity.id", cypher.StringLiteralCreate("7d729555-0d61-46ae-ab79-ce43e72f751b"))
	customer := cypher.CypherNewNode("Customer").NamedByString("c")
	relation := device.RelationshipTo(customer, "HAS")
	statement, _ := cypher.CypherMatch(relation).
...
func TestRenderComplexQuery(t *testing.T) {
	device := cypher.CypherNewNode("Farm").NamedByString("b")
	customer := cypher.CypherNewNode("Customer").NamedByString("c")
	relation := device.RelationshipTo(customer, "HAS")
	statement, _ := cypher.CypherMatch(relation).
		Returning(customer.GetSymbolicName()).
